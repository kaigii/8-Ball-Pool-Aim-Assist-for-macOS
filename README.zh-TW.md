...
| 功能 | 描述 | 效果 |
|------|------|------|
| ⚡ **即時畫面分析** | 擷取指定的遊戲區域，毫秒級處理 | 即時回應，無縫遊戲體驗 |
| 🔍 **進階影像處理** | 採用雙邊濾波、Canny 邊緣偵測等演算法，精準提取線條特徵 | 清晰準確的線條偵測 |
| 🎯 **雙重霍夫變換** | 分別針對長、短線段進行偵測，提高在不同視角下的偵測率 | 任何角度都能可靠偵測 |
| 🛡️ **點雲驗證機制** | 利用 RANSAC 思想，透過邊緣點雲的支持度來驗證和篩選最可靠的線段，排除雜訊干擾 | 無雜訊，準確結果 |
| ✨ **平滑線段追蹤** | 獨家的 LineTracker 模組，能消除線條的短暫閃爍，並提供優雅的淡出效果 | 專業穩定的視覺回饋 |
| 🪟 **無干擾懸浮視窗** | 使用 PySide6 建立的透明懸浮視窗，可疊加在遊戲之上，且完全穿透滑鼠事件 | 零干擾遊戲操作 |
| 🎛️ **即時參數調整器** | 內建一個強大的 GUI 調參工具 (tuner.py)，讓你可以即時微調所有視覺處理參數 | 完美客製化您的設定 |
